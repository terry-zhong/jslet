...
(function () {
  ...
  //------------------------------------------------------------------------------------------------------
    ...
    fldObj.label('هاتف العمل');
    fldObj.regularExpr(/(\(\d{3,4}\)|\d{3,4}-|\s)?\d{8}/ig, 'رقم الهاتف غير صالح!');//like: 0755-12345678
    dsEmployee.addField(fldObj);

    fldObj = jslet.data.createStringField('cellphone', 12);
    fldObj.label('الهاتف الخليوي');
    fldObj.regularExpr(/1\d{10}/ig, 'رقم الهاتف الخليوي غير صالحة!');//like: 13912345678
    dsEmployee.addField(fldObj);

    fldObj = jslet.data.createStringField('email', 30);
    fldObj.label('البريد الإلكتروني');
    fldObj.regularExpr(/^[a-zA-Z_0-9-'\+~]+(\.[a-zA-Z_0-9-'\+~]+)*@([a-zA-Z_0-9-]+\.)+[a-zA-Z]{2,7}$/ig, 'عنوان البريد الإلكتروني غير صحيح!');//like: foo@gmail.com
    dsEmployee.addField(fldObj);
