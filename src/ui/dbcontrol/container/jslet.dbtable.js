	...
	*/
	...
	_getSelectionFields: function(startColNum, endColNum) {
		...
		for(var i = startColNum; i <= endColNum; i++) {
			fldName = this.innerColumns[i].field;
			if(fldName) {
				fields.push(fldName);
			}
		}
...
//			'<div class="jl-tbl-curr-cell">&nbsp;</div>',
	...
	_moveColumn: function(rowIndex, srcCellIndex, destCellIndex) {
		...
		Z._changeColNum(rowObj, srcCellIndex, destCellIndex);
	},
	
	_changeColNum: function(rowObj, srcCellIndex, destCellIndex) {
		var srcCell = rowObj.cells[srcCellIndex],
			destCell = rowObj.cells[destCellIndex],
			srcColCfg = srcCell.jsletcolcfg,
			destColCfg = destCell.jsletcolcfg;
		srcColCfg.colNum = destColCfg.colNum;
		var cellObj, colCfg;
		if(srcCellIndex < destCellIndex) {
			for(var i = srcCellIndex + 1; i <= destCellIndex; i++) {
				colCfg = rowObj[i].jsletcolcfg;
				if(colCfg) {
					colCfg.colNum -= 1;
				}
			}
		} else {
			for(var i = destCellIndex; i < srcCellIndex; i++) {
				colCfg = rowObj[i].jsletcolcfg;
				if(colCfg) {
					colCfg.colNum += 1;
				}
...
jslet.ui.DefaultCellRender =  jslet.Class.create(jslet.ui.CellRender, {
	...
	refreshCell: function (cellPanel, colCfg) {
		...
		Z._showSelected(cellPanel.parentNode, fldName);
	} 
});
...
jslet.ui.SequenceCellRender = jslet.Class.create(jslet.ui.CellRender, {
	...
	refreshCell: function (cellPanel, colCfg) {
		...
		var jqDiv = jQuery(cellPanel), text;
		if(this._reverseSeqCol) {
			text = this._dataset.recordCount() - this._dataset.recno();
		} else {
			text = this._dataset.recno() + 1;
		}
