...
var swfobject = function() {
	...
	ua = function() {
		var w3cdom = typeof doc.getElementById != UNDEF
				...
				.toLowerCase(), p = nav.platform.toLowerCase(), windows = p
				? /win/.test(p)
	...
	onDomLoad = function() {
		...
		if ((typeof doc.readyState != UNDEF && doc.readyState == "complete")
				|| (typeof doc.readyState == UNDEF && (doc
						.getElementsByTagName("body")[0] || doc.body))) { // function
			// is
			// fired
			// after
			// onload,
	...
	var cleanup = function() {
		if (ua.ie && ua.win) {
			window.attachEvent("onunload", function() {
						...
						// cleanup dynamically embedded objects to fix
						// audio/video threads and force open sockets and
			...
			if (ua.w3 && !(ua.wk && ua.wk < 312) && swfUrlStr
				...
				addDomLoadEvent(function() {
							...
							if (attObj && typeof attObj === OBJECT) {
								for (var i in attObj) { // copy object to avoid
									// the use of
									// references, because
									// web authors often
									// reuse attObj for
							...
							if (parObj && typeof parObj === OBJECT) {
								for (var j in parObj) { // copy object to avoid
									// the use of
									// references, because
									// web authors often
									// reuse parObj for
							...
							if (flashvarsObj && typeof flashvarsObj === OBJECT) {
								for (var k in flashvarsObj) { // copy object
									// to avoid the
									// use of
									// references,
									// because web
							...
							if (hasPlayerVersion(swfVersionStr)) { // create
								// SWF
